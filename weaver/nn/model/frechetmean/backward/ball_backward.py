...
from weaver.nn.model.frechetmean.utils import d2arcosh, darcosh
...
def inverse_hessian(X, y, w, K):
    ...
    epsilon = 1e-5 
#     I = torch.eye(hessian.shape[-1], device=hessian.device)  # Identity matrix
#     stabilized_hessian = hessian + epsilon * I
    inv_hess = torch.inverse(hessian)
#     inv_hess = torch.linalg.pinv(hessian)
    return inv_hess
